	...
	if (isset($_POST["kitap_ver"])) {
        ...
        if ($barkod!=""&&$ogr_no!=""&&$turu!="") {
            ...
            if(!$ogr_bilgileri){
                ...
                $islem=$db->row("*","alinan_kitaplar",array("kitap_id"=>$kitap["id"],"ogrenci_id"=>$ogr_bilgileri["id"]));
                if (!$islem) {
                    $_DURUM['mesaj']="Sistemde bu kitabın verilme kaydı yok!";
                    echo "<script type='text/javascript'>$(function(){ $('#uyari-kutusu').addClass('hata').animate({opacity:'1',height:'30px'}, 1000)});</script>";
                }else{
                    $kayit=$db->update("alinan_kitaplar",array("veris_tarihi"=>date("Y-m-d H:i:s")),array("kitap_id"=>$kitap["id"],"ogrenci_id"=>$ogr_bilgileri["id"]));
                    if($kayit){
                        $db->update("kitaplar",array("kopya_sayisi"=>$kitap["kopya_sayisi"]+1),array("id"=>$kitap["id"]));
                        $_DURUM['mesaj']="İşlem tamam kitabı alabilirsiniz.";
